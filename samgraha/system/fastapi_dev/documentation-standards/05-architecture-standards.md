...
**Correct:**
> The `users` domain strictly separates HTTP concerns from business logic. The `UserRouter` extracts JWT tokens and delegates to `UserService`.

**Incorrect:**
> The `users` endpoint directly queries the database using SQLAlchemy.
> *Why wrong: Violates the layered architecture by bypassing the Service layer.*
