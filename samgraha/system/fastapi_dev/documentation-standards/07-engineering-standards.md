...
> *Deterministic rules for this domain: `audit/deterministic/document/07-engineering.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/05-purpose.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/03-build_standards.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/04-testing_standards.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/06-code_standards.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/07-constraints.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/08-traceability.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/01-guiding_principles.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/02-rationale.yaml`*
...
> *Structural rules: `audit/deterministic/section/07-engineering/11-async_patterns.yaml`*
...
```markdown
When engineering backend systems, standards must dictate:

* **Type Hinting:** Strict and pervasive use of type hints for all signatures.
* **Asynchronous I/O:** Proper use of async/await for network and database operations to avoid blocking the event loop.
* **Data Validation:** Centralized validation at the boundaries using robust schema definition.
* **Dependency Injection:** Use DI for database sessions, external services, and configuration.
* **Error Handling:** Normalize errors into standard HTTP responses before they leave the application.
```
...
**Correct:**
> All database calls must use async drivers (`asyncpg`) to prevent event loop blocking. Validation occurs strictly via Pydantic models at the router boundary.

**Incorrect:**
> We use `requests.get` to call the external API.
> *Why wrong: `requests` is synchronous and blocks the async event loop.*
...
- **Do:** Mandate async non-blocking I/O and strict type hinting.
- **Don't:** Allow global state or synchronous network calls.
...
**Correct:**
> All external APIs are versioned in the URL path (e.g., `/api/v1/users`).
...
**Correct:**
> Schema migrations are generated using Alembic and must be backward-compatible (expand-and-contract pattern) to support zero-downtime deployments.

**Incorrect:**
> Just run `alembic upgrade head` when you deploy.
> *Why wrong: Doesn't address backward compatibility.*
...
- **Structure:** paragraphs
- **Audience:** backend engineer
- **Do:** Mandate backward-compatible schema changes.
