...
```markdown
Backend build standards must address:

* **Dependency Management:** Package manager agnostic standards for reproducible builds (lockfiles).
* **Artifact Generation:** Building Docker images or serverless bundles.
* **API Validation:** Incorporating OpenAPI schema validation in the CI/CD pipeline.
* **Environment Configuration:** How configuration is injected at build or runtime.

The OpenAPI schema is automatically generated and validated during the build pipeline.

[Detail the CI/CD steps that generate the schema and validate it against the documented contract to catch breaking changes.]
```
...
**Correct:**
> The CI pipeline extracts the schema and runs `openapi-diff` to ensure no backward-incompatible changes were introduced to the API contract.
...
- **Do:** Automate API schema validation in CI.
