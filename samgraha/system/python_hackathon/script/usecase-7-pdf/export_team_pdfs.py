...
from db import get_canonical_domains
...
def collect_team_files(html_dir):
    ...
    if known_names is None:
        # Fallback: derive from {team}-summary.html files. Per-domain
        # summary pages (e.g. "{team}-ai-explanations-summary.html")
        # also match "*-summary.html" — exclude those or every team
        # fragments into one bucket per domain.
        import re
        domain_suffixes = tuple(
            f"-{d.replace('_', '-')}" for d in get_canonical_domains()
        )
        known_names = []
        for fp in all_files:
            basename = os.path.basename(fp)
            m = re.match(r"^(.+)-summary\.html$", basename)
            if m and not m.group(1).endswith(domain_suffixes):
