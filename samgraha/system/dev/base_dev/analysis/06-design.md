...
Guides the semantic-analysis step for the Design domain — read by
whichever agent runs it via MCP, on-demand, after `analyze_trend.py`
has produced `06-design-trend.json`.

## Version
1.0.0

## Analysis Intent
Design's score moving is only informative if the reader knows *why*.
This step turns scores + trend + findings into: a short narrative, chart
selection, and fix-plan decisions.

## Inputs
- `06-design-scores.json` (from `calculate.py`)
- `06-design-trend.json` (from `analyze_trend.py`)
- `06-design-validation.json`, `06-design-det-eval.json`,
  `06-design-sem-eval.json` (findings)

## Narrative Guidance
Design's audit rubric scores: Design Principles/UX Principles/
Accessibility consistency (C1), terminology consistency (C2), and
...
- If C1 (cross-section consistency) is failing, call out specifically
  which principles contradict — e.g. a UX pattern violates an
...
```json
{
  ...
  "focus_sections": ["design-principles", "ux-principles"],
  "reason": "..."
}
```
