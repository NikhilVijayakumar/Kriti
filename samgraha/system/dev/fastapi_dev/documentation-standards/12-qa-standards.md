...
```markdown
Backend QA standards must address:

* **Unit Testing:** Testing business logic in isolation using test runners and mocks.
* **Integration Testing:** Testing the API contract using HTTP clients against a test database.
* **Contract Testing:** Ensuring the API matches its OpenAPI specification.
* **Performance Testing:** Validating endpoint latency and throughput under load.

API endpoints are tested using [Test Client] within the [Test Framework] framework.
...
**Correct:**
> All endpoints are tested using an async HTTP client. A clean PostgreSQL test database is spun up per test session. Tests must assert both the HTTP 200 success path and the 422 validation error path.
