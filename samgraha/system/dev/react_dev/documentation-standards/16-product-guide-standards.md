...
Feature branch → Component → Tests → Review → Merge. Every feature gets its own branch. Components are built test-first. Reviews require accessibility and performance checks.

### Component Development Workflow

Design token alignment → Component implementation → Unit tests → Storybook stories → Visual regression → Accessibility audit → Performance audit → Merge.

### Design Token Contribution

Token changes require design review. New tokens must follow naming convention. Token deprecation process: mark deprecated, add replacement, remove after 2 releases.

### Storybook Documentation

Every component has stories covering: default state, all variants, interactive states, error states, loading states, empty states. Stories are the living documentation.

### Accessibility Review Checklist

Keyboard navigation verified, screen reader tested, color contrast checked, ARIA labels present, focus management working, live regions announcing.

### Performance Review Checklist

Bundle size impact assessed, lazy loading considered, memoization justified, image optimization verified, font loading strategy confirmed.
