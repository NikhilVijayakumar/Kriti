...
Vitest + @testing-library/react. Test categories: render tests (content renders correctly), interaction tests (user events handled), context tests (providers work correctly). Co-located test files.

### Visual Regression

Storybook-based visual diff. Chromatic or Percy for screenshot comparison. Baseline images stored in version control. CI gate prevents visual regressions.

### Accessibility Testing

axe-core integration in test suite. WCAG 2.1 AA scoring. Automated a11y CI gates. Manual testing checklists for screen readers.

### Cross-Browser Testing

Browser support matrix: last 2 versions of Chrome, Firefox, Safari, Edge. Responsive breakpoint validation at each tier. Mobile-first testing approach.

### Screen Reader Testing

Test with NVDA (Windows), VoiceOver (macOS), TalkBack (Android). Verify ARIA labels, live regions, focus management. Manual testing required — automation insufficient.

### Keyboard Navigation

Tab order verification. Focus trapping in modals. Escape key handling. Arrow key navigation in menus and lists. No keyboard traps.
