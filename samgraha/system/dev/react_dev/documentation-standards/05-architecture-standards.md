...
> *Deterministic rules for this domain: `audit/deterministic/document/05-architecture.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/01-purpose.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/02-system_overview.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/03-component_model.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/04-communication_paths.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/05-data_flow.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/06-security_considerations.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/07-rationale.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/08-constraints.yaml`*
...
> *Structural rules: `audit/deterministic/section/05-architecture/09-traceability.yaml`*
...
### Component Tree as Architecture

The component tree IS the architecture. Architectural decisions manifest as component hierarchy, not file structure. The component tree defines data flow, state ownership, and rendering boundaries.

### Module Boundaries

Module boundaries are enforced through barrel exports (`index.ts`). Each tier (atoms, molecules, organisms, templates) has its own barrel. Cross-tier imports flow downward only — atoms never import from organisms.

### Data Flow Architecture

Data flows unidirectionally: props-down, events-up. Context provides cross-cutting data. No bidirectional data flow within the same layer. No prop drilling through more than 2 levels without Context.

### State Architecture

State is divided into: cross-cutting state (Context) and UI-local state (useState). Context handles theme, language, auth. useState handles component-specific UI state. No external state libraries.

### Render Pipeline

Render optimization happens through: React.memo for expensive components, useMemo for expensive calculations, useCallback for stable function references, keyed lists for efficient reconciliation.

### Virtual DOM Concepts

Understanding reconciliation is architectural: keyed lists prevent unnecessary re-renders, component isolation limits render cascades, state colocation reduces render scope.
