...
Hooks co-located with primary consumer. Shared hooks in `hooks/` directory. One hook per file. `use*.ts` naming.

### Type Folder Structure

Complex prop types in `*Data.ts` files. Simple types inline. Interface exports for all component props. Generic type params documented.

### Barrel Export Conventions

Each tier has `index.ts`. Atoms barrel exports all atoms. Molecules barrel exports all molecules. Top-level barrel re-exports all tiers.
...
Feature index exports public API. Internal components are not exported.
