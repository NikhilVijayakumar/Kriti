...
Prerequisites (Node.js version, package manager), Installation steps, Development server startup, Build commands, Test commands, Storybook startup, Environment variables needed.

### Development Workflow

Feature branch → Component implementation → Tests → Storybook stories → Review → Merge. PR template includes accessibility checklist.

### Component Catalog Reference

README links to Storybook for component documentation. Storybook is the source of truth for component API and usage examples.

### Environment Setup

Clear instructions for `.env` file setup. Required environment variables documented with descriptions. No hardcoded defaults for secrets.
