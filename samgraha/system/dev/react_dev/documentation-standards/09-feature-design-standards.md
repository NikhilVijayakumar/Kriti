...
Frontend feature design combines three perspectives: user flow (navigation and interaction), component tree (UI hierarchy), and state map (data ownership and flow).

### Component Hierarchy Per Feature

Each feature defines which atoms, molecules, organisms, and templates it uses. New components are justified by reuse need, not organizational preference.

### State Ownership Per Feature

Each feature declares: which shared contexts it consumes, which local state it owns, which events it emits. State ownership prevents duplication and conflict.

### API Integration Patterns

Data fetching hooks per feature. Loading states per data requirement. Error states per API call. Optimistic updates where appropriate.

### Accessibility Per Feature

Keyboard navigation path through the feature. Screen reader announcements for dynamic content. ARIA roles for interactive elements. Focus management for modals and dialogs.
