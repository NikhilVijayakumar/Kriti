# Semantic Whole-Document Report — Engineering

**Document:** {{ document_path }}
**Standard:** `domain/07-engineering.md`
**Rubric:** `common/tier2/audit/semantic/document/engineering.md`
...
Verifies Engineering Documentation coheres as one set of repo-wide standards — Guiding Principles, Build Standards, Testing Standards, and Code Standards must not contradict each other. Section-level quality is owned by `audit/semantic/section/engineering/`; this file owns whether the pieces fit together as one coherent engineering approach.

## Scoring Criteria — Detail

### C1 — mandatory, 0 or 40: Guiding Principles, Build Standards, Testing Standards, and Code Standards are mutually consistent

**What this catches:** a principle implying a priority that another section's concrete rule contradicts; a Testing Standards threshold that Build Standards' pipeline doesn't actually enforce.
...
**What this catches:** the same tool, gate, or convention named differently in different sections — causing confusion for downstream Implementation references.
...
### C3 — recommended, 0 or 30: All Engineering documents in the domain cohere as one system

**What this catches:** two Engineering documents specifying incompatible build or test tooling for the same concern — a collection-wide check, not a within-one-document check.
...
{% endfor %}
{% else %}
No findings — document reads as one coherent engineering approach.
{% endif %}
...
| Rubric File | `common/tier2/audit/semantic/document/engineering.md` |
| Audit Date | {{ created_at }} |
| Revision | {{ revision_number }} |
| Session | {{ session_id }} |
