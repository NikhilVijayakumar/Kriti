# Semantic Whole-Document Report — Security

**Document:** {{ document_path }}
**Standard:** `domain/03-security.md`
**Rubric:** `common/tier2/audit/semantic/document/security.md`
...
| C2 — Data class/Protection alignment | mandatory | 30 | {{ results['C2'].previous_passed_display | default('—') }} | {{ results['C2'].passed_display }} | {{ results['C2'].trend_display }} |
...
Verifies Security Documentation coheres as one threat model — Threat Model, Data Classification, and Security Principles must not contradict each other. Section-level quality is owned by `audit/semantic/section/security/`; this file owns whether the pieces fit together as one coherent security posture.

## Scoring Criteria — Detail

### C1 — mandatory, 0 or 40: Threat Model, Data Classification, and Security Principles are mutually consistent

**What this catches:** a Security Principle contradicted by how Threat Model actually treats a threat; a Data Classification tier with no corresponding protections in Security Principles.
...
**What this catches:** a Data Classification tier defined without any protection requirement — a gap between what's classified and what's actually guarded.
...
**What this catches:** the same asset or threat category named differently in different sections — undermining Compliance's obligation-to-control traceability.
...
{% endfor %}
{% else %}
No findings — document reads as one coherent security posture.
{% endif %}
...
| Rubric File | `common/tier2/audit/semantic/document/security.md` |
| Audit Date | {{ created_at }} |
| Revision | {{ revision_number }} |
| Session | {{ session_id }} |
