...
> **Source:** `domain/07-engineering.md` §Code Standards
...
**Correct:**
> **Style Guide:** All modules follow the Rust Style Guide (rustfmt defaults). Functions are named with snake_case convention. Files are named after the module they contain. **Linting Configuration:** Clippy is configured via `Cargo.toml` under `[lints]` or `clippy.toml` at the repository root. All CI builds must pass `cargo clippy --all-targets --all-features -- -D warnings` before merge.
