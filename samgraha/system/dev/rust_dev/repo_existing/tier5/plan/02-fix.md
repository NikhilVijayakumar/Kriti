...
No difference vs repo_existing_no_doc/tier5 (identical after its one-time bootstrap-readme usecase). vs repo_new/tier5: this workflow audits first - no generate/migrate stage, per-domain create runs only where the audit finds no conforming doc - but the fix-or-create procedure is shared.