...
- **Section-level fix** if ALL of the following are true:
  ...
  Apply via `tier1/templates/generation/section/{domain}/{section}.md`'s `## Audit Fix` slot for each failing section.

- **Whole-document regeneration** otherwise (failures spread across 3+ sections, OR any whole-document criterion failed).
  
  Apply via `tier1/templates/generation/document/{domain}.md` → regenerate the full document, incorporating the findings from the audit report.
