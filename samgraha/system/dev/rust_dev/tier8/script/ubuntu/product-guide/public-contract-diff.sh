...
while IFS= read -r -d '' file; do
    ...
    [[ -n "$found" ]] && ACTUAL_EPS+=$'\n'"$found"
    found=$(grep -oE '#\[(get|post|put|patch|delete|head|options)\s*\(\s*"(/[\w/{}\-\.]+)"\s*\)\]' "$file" 2>/dev/null || true)
    [[ -n "$found" ]] && ACTUAL_EPS+=$'\n'"$found"
    found=$(grep -oE '\.route\s*\(\s*"(/[\w/{}\-\.]+)"\s*,\s*(get|post|put|patch|delete)\s*\(' "$file" 2>/dev/null || true)
    [[ -n "$found" ]] && ACTUAL_EPS+=$'\n'"$found"
done < <(find "$REPO_ROOT" -type f \( -name "*.rs" -o -name "*.ts" -o -name "*.js" -o -name "*.py" -o -name "*.go" -o -name "*.java" -o -name "*.cs" -o -name "*.rb" \) -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/__pycache__/*" -not -path "*/.venv/*" -not -path "*/dist/*" -not -path "*/build/*" -not -path "*/vendor/*" -not -path "*/.opencode/*" -print0 2>/dev/null)
