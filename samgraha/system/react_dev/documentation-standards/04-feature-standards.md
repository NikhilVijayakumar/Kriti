...
A frontend feature consists of: component(s) + hook(s) + type(s) + test(s). All four artifacts must be present for a feature to be complete.

### Feature Folder Structure

Features are organized in dedicated folders: `feature/FeatureName/` containing `components/`, `hooks/`, `types/`, and `__tests__/` subdirectories.

### Feature Isolation

Features must be isolated from each other. Cross-feature communication happens through shared contexts or URL state, not direct imports between feature folders.

### Feature-Level State Ownership

Each feature owns its local state. Cross-cutting state (theme, language, auth) lives in shared contexts. Features consume shared state via hooks, never directly importing contexts.

### Feature Composition

Features compose through URL routing and shared layout components. Feature components are not imported directly by other features — use shared components instead.
