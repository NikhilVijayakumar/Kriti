...
TypeScript strict mode required. No `any` types. All props, state, hooks, and return types explicitly typed. Interface exports for all component props.

### Functional Components Only

No class components (except ErrorBoundary). All components are functions with `FC<Props>` typing. Explicit `ReactElement` return type.

### Hooks for Everything

All state and side effects use hooks. No class lifecycle methods. Custom hooks for reusable logic. Hook naming: `use*` prefix. No conditional hook calls.

### Named Exports Only

No default exports. All exports are named. Barrel exports at each tier index.

### Props Patterns

- Action config objects: `{ label, variant, onClick }`
- Render props: `render?: (row: T) => ReactNode`
- Composition slots: `title`, `children`, `action`
- Optional with defaults: `size = 10`
- Generic type params: `DataTable<T extends Record<string, unknown>>`
- Union literals: `StatusDotTone = 'ok' | 'warning' | 'error'`

### Error Boundary Pattern

Class component wrapping dynamic renderers. Props: `children`, `fallback?: ReactNode`, `localizedFallbackText?: string`. Cell-level wrapping in data displays.

### i18n Pattern

`useLanguage()` hook returns `{ literal }`. Every user-visible string uses `literal['key']` with `??` fallback. Provider wraps app root. Language stored in Context.
