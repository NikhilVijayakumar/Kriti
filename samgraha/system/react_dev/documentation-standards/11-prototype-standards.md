...
Frontend prototypes are throwaway HTML/React, not production code. They validate design decisions before implementation begins.

### Storybook as Prototype Surface

Storybook stories serve as living prototypes. Each story is a self-contained UI state. Stories are disposable — they validate, not persist.

### Mock Data Patterns

Prototypes use typed fixture data. Mock data follows the same interfaces as production data. Fixtures are co-located with prototype stories.

### User Testing with Prototypes

Prototypes enable user testing before code is written. Test with real users, not engineers. Capture feedback in design documentation.

### Prototype → Production Conversion

Conversion checklist: replace mock data with real hooks, add error boundaries, add loading states, add accessibility, add tests. Never promote prototype code directly.
