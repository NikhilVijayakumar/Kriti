...
> *Deterministic rules for this domain: `audit/deterministic/document/03-security.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/04-purpose.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/01-threat_model.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/02-data_classification.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/03-security_principles.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/07-compliance.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/08-incident_response.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/05-constraints.yaml`*
...
> *Structural rules: `audit/deterministic/section/03-security/06-traceability.yaml`*
...
All user input must be sanitized before rendering. Never use `dangerouslySetInnerHTML` with untrusted content. Use framework-provided escaping mechanisms. CSP headers must be configured to prevent inline scripts.

### CSRF Protection

API calls must include CSRF tokens or use SameSite cookie patterns. State-changing operations must use POST/PUT/DELETE with proper token validation.

### Dependency Auditing

Client-side dependencies must be audited regularly. Use `npm audit` or equivalent in CI. No known critical vulnerabilities in production dependencies.

### Secret-Free Client Code

No secrets, API keys, or credentials in client-side code. Environment variables exposed to the client must be explicitly marked as public. Sensitive operations must go through server-side proxies.

### Content Security Policy

CSP headers must be configured to restrict script sources, prevent inline execution, and block unknown origins. CSP violations must be reported and monitored.
