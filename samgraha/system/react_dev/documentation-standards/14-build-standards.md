...
Vite as default dev server. Hot module replacement. TypeScript path aliases. Environment variable handling via `import.meta.env`.

### Code Splitting

Route-based splitting via React.lazy + Suspense. Component-based splitting for heavy libraries. Shared chunks for common dependencies.

### Tree Shaking

ES modules required for tree shaking. Side-effect-free declarations. No `import *` in production code. Named imports only.

### Asset Pipeline

Images: optimized, WebP/AVIF preferred. Fonts: self-hosted, woff2 format. Icons: SVG sprite or component-based. Static assets in `public/` directory.

### Environment Configuration

`.env` for local development. `.env.production` for production defaults. `.env.staging` for staging. No secrets in client-exposed variables.

### Bundle Size Gates

Per-route budgets. Per-component budgets for shared chunks. CI enforcement with size limits. Alert on budget exceeded.

### Source Map Strategy

Source maps in development and staging. No source maps in production. Error tracking service receives source maps privately.
