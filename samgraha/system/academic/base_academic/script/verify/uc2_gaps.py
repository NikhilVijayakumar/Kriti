"""Verify script for gap-analysis — checks usecase completion criteria."""
import sys
...
def main():
    args = parse_args(description="Verify gap-analysis completion")
    conn = academic_schema.get_conn(args.db_path)
    try:
        complete, detail = academic_schema.usecase_status(
            conn, args.paper_id, "gap-analysis")
