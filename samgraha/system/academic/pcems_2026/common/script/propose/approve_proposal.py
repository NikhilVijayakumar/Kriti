...
def main():
    ...
    try:
        ...
        # Supersede any prior pending reviews for this (paper, phase, scope, map_kind)
        conn.execute(
            "UPDATE academic_proposal_review SET is_latest=0 "
            "WHERE paper_id=? AND phase=? AND scope_domain_id IS ? AND map_kind IS ?",
            (payload["paper_id"], phase, domain_id, map_kind))
        review_status = "rejected" if reject else "approved"
        conn.execute(
            "INSERT INTO academic_proposal_review "
            "(proposal_id, paper_id, phase, map_kind, scope_domain_id, review_status, "
            " source, user_comment, iteration, summary, content_md, "
            " computed_context) "
            "VALUES (?,?,?,?,?,?,'approve-proposal',?,?,?,?,?)",
            (proposal_id, payload["paper_id"], phase, map_kind, domain_id,
             review_status, reason,
             meta.get("iteration", 0),
             meta.get("summary", ""),
             meta.get("content_md", ""),
              json.dumps(meta.get("computed_context"))))
        conn.commit()
    finally:
        conn.close()
