...
Reads common/schema-manifest/standard.yaml, creates academic_* tables, seeds
...
PCEMS_ROOT = SCRIPT_DIR.parent.parent  # pcems_2026/ (seeder.py lives at common/script/)
sys.path.insert(0, str(SCRIPT_DIR.parent / "common" / "script"))
...
_DIAGRAM_ARCHITECTURE_STEPS = [
    ...
    {"order": 2, "kind": "deterministic",
     ...
     "script": "gather-module-evidence"},
    {"order": 3, "kind": "semantic",
     "description": "Analyze each module for architecture",
     "prompt": "module-analysis-architecture"},
    {"order": 4, "kind": "deterministic",
     "description": "Persist per-module architecture analysis",
     "script": "persist-module-analysis"},
    {"order": 5, "kind": "deterministic",
     "description": "Gather cross-module evidence",
     "script": "gather-cross-module-evidence"},
...
_RENDER_PAPER_STEPS = [
    {"order": 1, "kind": "deterministic",
     "description": "Extract findings' tables/figures into standalone files",
     "script": "extract-tables-figures"},
    {"order": 2, "kind": "deterministic",
     "description": "Assemble all domain drafts into one Markdown document",
     "script": "assemble-markdown-document"},
    {"order": 3, "kind": "deterministic",
...
# tables/figures generation — same 3-step shape as
# _CROSS_CUTTING_GENERATION_STEPS, but the gather step reads the findings
# domain's current draft (gather-tables-figures-evidence) instead of
# academic_cross_module_analysis — neither domain has an upstream module
# analysis usecase, so there is no cross_module_analysis row to read.
_TABLES_FIGURES_GENERATION_STEPS = [
    {"order": 1, "kind": "deterministic",
     "description": "Gather findings draft as {domain} evidence",
     "script": "gather-tables-figures-evidence"},
    {"order": 2, "kind": "semantic",
     "description": "Generate {domain} audit draft from findings",
     "prompt": "generate-{domain}"},
    {"order": 3, "kind": "deterministic",
     "description": "Persist {domain} section draft",
     "script": "persist-section-draft"},
...
def _make_tables_figures_steps(domain):
    """Create pre-substituted step list for the tables/figures generate usecase."""
    return [
        {k: (v.replace("{domain}", domain) if isinstance(v, str) else v)
         for k, v in s.items()}
        for s in _TABLES_FIGURES_GENERATION_STEPS
    ]
...
_WHOLE_DOCUMENT_STEP_MAP = {
    ...
    "generate-section-draft-tables": _make_tables_figures_steps("tables"),
    "generate-section-draft-figures": _make_tables_figures_steps("figures"),
}
...
def _read_prompt_content(location, standard_root):
    ...
    location is relative to common/schema-manifest/ (the YAML's own directory).
    standard_root is the standard's root directory.
    Tries relative to the YAML's directory, then falls back to absolute."""
    yaml_dir = standard_root / "common" / "schema-manifest"
    loc = Path(location)
    if loc.is_absolute():
        target = loc
    else:
...
def main():
    ...
    for s in spec.get("scripts", []):
        ...
        abs_loc = str((SCRIPT_DIR.parent / "schema-manifest" / location).resolve())
