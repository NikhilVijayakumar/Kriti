...
-- Aggregated Step 0 verification verdict per model — PASS/FAIL rate per
-- (paper, check_kind, model).  Enables multi-model comparison: "3 of 4
-- models agree this claim is grounded" without digging through raw rows.
CREATE VIEW IF NOT EXISTS academic_v_step0_verdict_by_model AS
SELECT
    f.paper_id,
    f.check_kind,
    f.model,
    COUNT(*) AS total_checks,
    SUM(CASE WHEN f.verdict = 'PASS' THEN 1 ELSE 0 END) AS pass_count,
    SUM(CASE WHEN f.verdict = 'FAIL' THEN 1 ELSE 0 END) AS fail_count,
    ROUND(CAST(SUM(CASE WHEN f.verdict = 'PASS' THEN 1 ELSE 0 END) AS REAL) / CAST(COUNT(*) AS REAL), 3) AS pass_rate
FROM academic_step0_claim_findings f
GROUP BY f.paper_id, f.check_kind, f.model;
