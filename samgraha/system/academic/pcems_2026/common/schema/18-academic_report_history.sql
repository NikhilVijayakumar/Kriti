...
CREATE TABLE IF NOT EXISTS academic_report_history (
    ...
    report_kind      TEXT    NOT NULL DEFAULT 'paper' CHECK (report_kind IN ('paper','audit-deterministic','audit-semantic','audit-summary','proposal','section-draft')),
    format           TEXT    NOT NULL CHECK (format IN ('markdown','html','pdf','docx')),
    final_score      REAL,
    score_band       TEXT,
    scope_domain_id  INTEGER REFERENCES academic_domains(id),
    map_kind         TEXT    CHECK (map_kind IS NULL OR map_kind IN ('citation','figure','equation','table')),
