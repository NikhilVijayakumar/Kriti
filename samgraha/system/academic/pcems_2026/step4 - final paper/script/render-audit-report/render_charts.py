...
def _record_chart(conn, chart_key, paper_id, fpath, commit_sha, params=None, content_hash=None):
    ...
    width/height come from the actual saved PNG (Section0's bbox_inches="tight"
...
# ---------------------------------------------------------------------------
# New chart types - Section5 of the template+visualization depth proposal
# ---------------------------------------------------------------------------
...
def _pipeline_progress_matrix(plt, paper_id, conn, domains, output_path):
    """12x9 heatmap: domains x pipeline stages, PASS(green)/FAIL(red)/pending(grey)."""
    import numpy as np
