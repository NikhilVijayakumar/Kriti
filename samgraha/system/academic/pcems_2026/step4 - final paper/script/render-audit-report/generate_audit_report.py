...
templates from score/audit/plagiarism results. Produces 12x6 per-domain reports
...
def _status_class(status_str):
    ...
    if status_str == "PASS" or status_str == "Y":
        return "pass"
    if status_str == "FAIL" or status_str == "N":
        return "fail"
    return "pending"
...
def _get_pipeline_progress_data(conn, paper_id, domains):
    """Build the 12x9 domain-by-stage pipeline-progress matrix plus
    ...
    for drow in domain_rows:
        if all(drow[k] == "Y" for k, _ in _PIPELINE_STAGES):
            complete_count += 1
...
def main():
    ...
    try:
        ...
        # --- Output directory ---
        output_dir = os.path.join(str(repo_root), ".samgraha", "output",
                                  "draft", f"paper-{paper_id}", "audit")
