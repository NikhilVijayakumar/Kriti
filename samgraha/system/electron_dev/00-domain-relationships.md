...
Cross-domain dependency map for the desktop engineering documentation standards in this directory — the agreed target model for how desktop systems are engineered. Use it to see what a domain feeds into, or what feeds into it, without opening every standard document.

This file is system-specific to `electron_dev`. It reflects desktop engineering domain relationships, not generic software engineering.
...
### Desktop-Specific Domain Concerns

Each domain has desktop-specific focus areas that distinguish this system from generic software engineering:

| Domain | Desktop-Specific Concern |
|--------|--------------------------|
| 01-vision | Process isolation as architectural principle, native platform integration, offline-first capability |
| 02-philosophy | Strict process boundaries, IPC abstraction, service registry, no direct DOM from Main |
| 03-security | Process isolation security, context isolation, Node.js API restrictions, code signing |
| 04-feature | Feature = process component + IPC channels + service + tests |
| 05-architecture | Three-process architecture (Main/Renderer/Preload), IPC patterns, storage domains |
| 06-design | Design tokens, native window chrome, platform-specific styling, dark mode |
| 07-engineering | Electron/Node.js patterns, service registry, event system, background jobs |
| 08-external-context | IPC framework, storage engines, auto-update, packaging tools |
| 09-feature-design | Desktop UI feature design, window management, multi-window flows |
| 10-feature-technical | Process implementation, IPC implementation, service lifecycle |
| 11-prototype | Mock IPC channels, throwaway Electron apps, prototype data |
| 12-qa | IPC testing, process isolation testing, cross-platform testing |
| 13-implementation | Process folder structure, service patterns, IPC organization |
| 14-build | electron-builder, code signing, auto-update, platform distribution |
| 15-readme | Desktop README, setup instructions, development workflow |
| 16-product-guide | Service catalog, IPC documentation, cross-platform testing |
...
* **Implementation** is fully specified: the as-built, one-to-one counterpart to Feature Technical, distinct from Engineering's Code Standards by scope (per-feature vs repo-wide). See `13-implementation-standards.md`'s "Relationship to Engineering's Code Standards."
...
This matrix shows which domains depend on which. Rows depend on columns.

|  | vision | philosophy | security | feature | architecture | design | engineering | external-context |
|--|--------|------------|----------|---------|--------------|--------|-------------|------------------|
| **feature-design** | | | | ✓ | | ✓ | | ✓ |
| **feature-technical** | | | | ✓ | ✓ | | ✓ | ✓ |
| **prototype** | | | | | | | | |
...
### Desktop Engineering Tier Summary

| Tier | Purpose | Desktop Focus |
|------|---------|---------------|
| 1 | Foundational | Vision and philosophy — why we build desktop systems this way |
| 2 | Independent | Security, feature definition, architecture, design, engineering, external context — each a standalone concern |
| 3 | Derived | Feature design and technical — combines multiple Tier 2 outputs into per-feature plans |
| 4 | Validation | Prototype — validates design before anything is built |
| 5 | Realization | Implementation — builds the actual processes, IPC channels, services |
| 6 | Validation | QA — validates the built artifact (IPC tests, process tests, cross-platform) |
| 7 | Packaging | Build — packages, signs, distributes |
| 8 | Final | README and Product Guide — documentation for developers and users |
